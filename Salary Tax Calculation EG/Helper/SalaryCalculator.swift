...
class SalaryCalculator {
    ...
    static let exemptionLimit = 15000.0

    private static func getTaxLayers(yearlySalary: Double) -> [[Double]] {
        var layers = [[Double]]()
        if 0 <= yearlySalary && yearlySalary < 600000 {
            layers = [[self.exemptionLimit, 0], [15000, 0], [15000, 2.5], [15000, 10], [15000, 15], [140000, 20], [200000, 22.5], [200000, 25]]
        } else if 600000 <= yearlySalary && yearlySalary < 700000 {
            layers = [[self.exemptionLimit, 0], [30000, 2.5], [15000, 10], [15000, 15], [140000, 20], [200000, 22.5], [300000, 25]]
        } else if 700000 <= yearlySalary && yearlySalary < 800000 {
            layers = [[self.exemptionLimit, 0], [45000, 10], [15000, 15], [140000, 20], [200000, 22.5], [400000, 25]]
        } else if 800000 <= yearlySalary && yearlySalary < 900000 {
            layers = [[self.exemptionLimit, 0], [60000, 15], [140000, 20], [200000, 22.5], [500000, 25]]
        } else if 900000 <= yearlySalary && yearlySalary < 1000000 {
            layers = [[self.exemptionLimit, 0], [200000, 20], [200000, 22.5], [600000, 25]]
        } else if 1000000 <= yearlySalary {
            layers = [[self.exemptionLimit, 0], [400000, 22.5], [yearlySalary - 400000, 25]]
        }
